...
def showTopNPreds(pred,class_idx,labels,topN = 5):
  # flatten arrays
  p_flat = pred.squeeze().flatten()
  c_flat = class_idx.squeeze().flatten()
  
  # zip and convert to list, then sort by descending order of prob
  pc_list = list(zip(p_flat,c_flat))
  pc_list.sort(key=lambda c: c[0], reverse=True)
  ...
  for p,c in pc_list:
    if cnt >= topN:
      # print('break when cnt=',cnt)
      break
    # print('cnt=',cnt)
    if c not in classItems:
      cnt+=1
      classItems.append(c)
      print("Label:[{}], Probability:[{:0.03}%]".format(labels[c.item()],100*p.item()))
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        print('Predicted Class:[{}], Value:[{}]'.format(labels[predicted_class], predicted_class.item()))
        
        showTopNPreds(pred,class_idx,labels,topN=5)
        ...
        cv2.imshow("scaled_score_map", imutils.resize(score_map, width=displayWidth))
